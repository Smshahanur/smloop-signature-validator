        ...
        with open(tmp_path, "rb") as inf:
            ...
            for sig in embedded:
                try:
                    ...
                    # Signature validity and certificate trust are separate.
                    # A signature is VALID when its signed bytes are intact and
                    # the cryptographic signature check succeeds. An unavailable
                    # trust chain must not downgrade a cryptographically valid
                    ...
                    try:
                        ...
                        try:
                            signing_time = iso(
                                getattr(status, "signing_time", None)
                            )
                        except Exception:
                            pass
                    ...
                    if intact and valid and not trusted:
                        reason_parts.append(
                            ...
                            "Certificate trust could not be fully established "
                            "on this server."
                        )
                    ...
                    signatures.append({
                        ...
                        "certificate_status": (
                            "Trusted"
                            if trusted
                            else ("Untrusted" if valid else "Invalid")
                        ),
                    ...
                    signatures.append({
                        ...
                        "reason": (
                            f"Could not fully validate this signature: {exc}"
                        ),
                    })
